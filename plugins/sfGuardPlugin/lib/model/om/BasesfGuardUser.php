...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Bookmark[] Collection to store aggregation of Bookmark objects.
	 */
	protected $collBookmarks;

	/**
	 * @var        Criteria The criteria used to select the current contents of collBookmarks.
	 */
	private $lastBookmarkCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collBookmarks = null;
			$this->lastBookmarkCriteria = null;
	...
	/**
	 * Clears out the collBookmarks collection (array).
	 *
	...
	{
		$this->collBookmarks = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collBookmarks collection (array).
	 *
	 * By default this just sets the collBookmarks collection to an empty array (like clearcollBookmarks());
	...
	/**
	 * Gets an array of Bookmark objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Bookmarks from storage. If this sfGuardUser is new, it will return
	 ...
	 * @throws     PropelException
	 */
	public function getBookmarks($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collBookmarks === null) {
			if ($this->isNew()) {
			   $this->collBookmarks = array();
				...
				$criteria->add(BookmarkPeer::USER_ID, $this->id);
	...
	/**
	 * Returns the number of related Bookmark objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Bookmark objects.
	 * @throws     PropelException
	 */
	public function countBookmarks(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
		...
		if ($this->collBookmarks === null) {
			if ($this->isNew()) {
				...
				$count = BookmarkPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastBookmarkCriteria) || !$this->lastBookmarkCriteria->equals($criteria)) {
					...
					$count = count($this->collBookmarks);
				}
			} else {
				$count = count($this->collBookmarks);
			}
	...
	/**
	 * Method called to associate a Bookmark object to this object
	 * through the Bookmark foreign key attribute.
	 *
	 * @param      Bookmark $l Bookmark
	 * @return     void
	 * @throws     PropelException
	 */
	public function addBookmark(Bookmark $l)
	{
		...
		if (!in_array($l, $this->collBookmarks, true)) { // only add it if the **same** object is not already associated
			array_push($this->collBookmarks, $l);
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Votes from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collVotes === null) {
			if ($this->isNew()) {
			   $this->collVotes = array();
				...
				$criteria->add(VotePeer::USER_ID, $this->id);
	 ...
	 */
	public function getVotesJoinBookmark($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collVotes === null) {
			if ($this->isNew()) {
				...
				$this->collVotes = VotePeer::doSelectJoinBookmark($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	{
		if ($deep) {
			if ($this->collBookmarks) {
				foreach ((array) $this->collBookmarks as $o) {
